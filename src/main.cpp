...
/* 
enum wl_enc_type { 
  // https://stackoverflow.com/questions/62784829/c-problem-with-vector-iterator-on-esp8266-microcontroller
    ENC_TYPE_NONE = 1,
    ENC_TYPE_WEP  = 2,
    ENC_TYPE_TKIP = 3,
    ENC_TYPE_CCMP = 4,
    ENC_TYPE_AUTO = 5,
    ENC_TYPE_FINAL_ANSWER = 42
};

const char* getEncryptionType(int this_type) {
  // convert the enum to the apropriate string
  std::string enc = "Unknown";
  if (this_type == ENC_TYPE_WEP) enc = "WEP";
  else if (this_type == ENC_TYPE_TKIP) enc = "WPA";
  else if (this_type == ENC_TYPE_CCMP) enc = "WPA2";
  else if (this_type == ENC_TYPE_NONE) enc = "None, Open Network";
  else if (this_type == ENC_TYPE_AUTO) enc = "Auto";
  return enc.c_str();
} 
